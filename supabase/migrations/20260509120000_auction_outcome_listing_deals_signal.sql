-- auction_outcome: deal_opened only when listing_deals exists (auction-level signal).
-- Without listing_deals: no_bids, threshold_not_met, or leave pending until deal row exists.
...
begin
  ...
  if v_has_deal then
    ...
    elsif v_high < ceil((v_reserve * v_pct) / 100.0) then
      v_next := 'threshold_not_met';
    else
      return 'pending';
    end if;
