-- Explicit post-auction result on listings (may already exist on remote).
alter table public.listings
  add column if not exists auction_outcome text not null default 'pending';

alter table public.listings
  drop constraint if exists listings_auction_outcome_check;

alter table public.listings
  add constraint listings_auction_outcome_check check (
    auction_outcome in (
      'pending',
      'no_bids',
      'threshold_not_met',
...
-- Resolve pending → terminal outcome when the auction has ended (matches app derive rules).
