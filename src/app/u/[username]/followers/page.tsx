...
export default async function FollowersPage({ params }: PageProps) {
  ...
  if (followsError) throw new Error(`Could not load followers: ${followsError.message}`);
  ...
  let followerProfiles: FollowProfileRow[] = [];
  const ratingByUserId = new Map<string, string>();

  if (followerIds.length > 0) {
    const { data: followerRows, error: followerProfilesError } = await supabase
      .from("profiles")
      .select("id, username, display_name, active_title")
      .in("id", followerIds);

    if (followerProfilesError) {
      throw new Error(`Could not load follower profiles: ${followerProfilesError.message}`);
    }

    const byId = new Map(
      ((followerRows ?? []) as FollowProfileRow[]).map((row) => [String(row.id), row]),
    );
    followerProfiles = followerIds
      .map((id) => byId.get(id))
    ...
    const { data: ratingRows, error: ratingsError } = await supabase
      ...
      .in("to_user_id", followerIds);
    if (ratingsError) throw new Error(`Could not load follower ratings: ${ratingsError.message}`);
  ...
  return (
    <div className={pageShellClass}>
      <header className={pageHeaderClass}>
        ...
        <div>
          <h1 className={pageTitleClass}>Følgere</h1>
